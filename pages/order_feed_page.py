...
class OrderFeedPage(BasePage):
    ...
    @allure.step("Проверить увеличение счётчика 'Выполнено за всё время'")
    def is_all_time_counter_increased(self, initial_counter, timeout=15):
        """Проверяет, увеличился ли счётчик 'Выполнено за всё время'."""
        def counter_increased(driver):
            current = self.get_all_time_counter()
            return current > initial_counter
        self.wait.until(counter_increased, message=f"Счётчик 'Выполнено за всё время' не увеличился: {initial_counter}")
        return True
    ...
    allure.step("Проверить увеличения счётчика 'Выполнено за сегодня'")
    def is_today_counter_increased(self, initial_counter, timeout=15):
        def counter_increased(driver):
            current = self.get_today_counter()
            return current > initial_counter
        self.wait.until(counter_increased, message=f"Счётчик 'Выполнено за сегодня' не увеличился: {initial_counter}")
        return True
